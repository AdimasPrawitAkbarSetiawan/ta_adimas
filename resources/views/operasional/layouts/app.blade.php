...
<head>
    ...
    <style>
        * { font-family: 'Poppins', sans-serif; }
        ...
        .nav-item:hover { background: rgba(255,255,255,0.55); color: #222; }
        .nav-item.active { background: #3b5bdb; color: white !important; }
        ...
        .main-content { margin-left: 200px; padding: 18px 22px; min-height: 100vh; }
        .topbar {
            display: flex;
...
<div class="sidebar">
    ...
    <a href="{{ route('operasional.dashboard') }}"
       class="nav-item {{ request()->routeIs('operasional.dashboard') ? 'active' : '' }}">
    ...
    <a href="{{ route('operasional.proyek-berjalan.index') }}"
       class="nav-item {{ request()->routeIs('operasional.proyek-berjalan.*') || request()->routeIs('operasional.input-progres.*') ? 'active' : '' }}">
    ...
    <a href="{{ route('operasional.proyek-disetujui.index') }}"
       class="nav-item {{ request()->routeIs('operasional.input-kebutuhan.*') ? 'font-bold' : '' }}">
    ...
    <a href="{{ route('operasional.proyek-berjalan.index') }}"
       class="nav-item {{ request()->routeIs('operasional.input-progres.*') ? 'font-bold' : '' }}">
    ...
    <div class="user-card">
        ...
        <form method="POST" action="{{ route('logout') }}" class="ml-auto flex-shrink-0">
            @csrf
            <button type="submit" title="Keluar" class="text-gray-400 hover:text-red-500">
...
<div class="main-content">
    <div class="topbar">
        <h1 class="text-base font-semibold text-gray-500">@yield('judul-halaman')</h1>
...
function toggleNotif() { document.getElementById('notif-dropdown').classList.toggle('hidden'); loadNotifications(); }
...
function markAllRead() {
    ...
    .then(() => { loadNotifications(); document.getElementById('notif-badge').classList.add('hidden'); });
}

function toggleChatPopup() {
    const popup = document.getElementById('chat-popup');
    if (popup.classList.contains('hidden')) { popup.classList.remove('hidden'); loadChatUsers(); }
    else { closeChatPopup(); }
}
...
function loadChatUsers() {
    fetch('/chat/users').then(r => r.json()).then(users => {
        ...
        if (users.length === 0) { list.innerHTML = '<p class="text-center text-gray-400 text-xs py-6">Tidak ada user.</p>'; return; }
...
function openChatWith(userId, userName) {
    ...
    wrap.classList.remove('hidden'); wrap.style.display = 'flex'; wrap.style.flexDirection = 'column'; wrap.style.flex = '1'; wrap.style.overflow = 'hidden';
    loadChatMessages();
    if (chatPollingInterval) clearInterval(chatPollingInterval);
...
function backToUserList() {
    ...
    const wrap = document.getElementById('chat-messages-wrap');
    wrap.classList.add('hidden'); wrap.style.display = 'none';
    loadChatUsers();
}
...
function sendChatMsg() {
    ...
    fetch('/chat/send', { method: 'POST', headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': '{{ csrf_token() }}' }, body: JSON.stringify({ receiver_id: activeChatUserId, message }) })
...
function pollBadges() {
    fetch('/notifications/unread').then(r => r.json()).then(data => {
        ...
        if (data.count > 0) { badge.textContent = data.count; badge.classList.remove('hidden'); } else { badge.classList.add('hidden'); }
    });
