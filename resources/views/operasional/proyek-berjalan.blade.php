...
@section('judul-halaman', 'Daftar Proyek Berjalan')
        ...
        <table class="w-full text-sm" id="tabelProyek">
            <tbody>
                ...
                <tr class="border-b border-gray-100 hover:bg-gray-50 transition">
                    ...
                    <td class="px-4 py-3">
                        <span class="bg-yellow-400 text-white text-xs font-semibold px-3 py-1 rounded-full">Berjalan</span>
                    </td>
                    <td class="px-4 py-3 text-gray-500 text-xs">{{ \Carbon\Carbon::parse($proyek->created_at)->format('d-m-Y') }}</td>
                    <td class="px-4 py-3">
                        <a href="{{ route('operasional.input-progres.create', $proyek->id) }}"
                           class="bg-blue-100 text-blue-600 text-xs font-semibold px-3 py-1 rounded-full hover:bg-blue-200 whitespace-nowrap">
                ...
                @empty
                <tr>
                    <td colspan="6" class="text-center py-8 text-gray-400">Belum ada proyek yang berjalan.</td>
                </tr>
                @endforelse
            </tbody>
        </table>
