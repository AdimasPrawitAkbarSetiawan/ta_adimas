...
<div class="bg-white rounded-2xl shadow-sm p-6">
    ...
    <div class="flex items-center justify-between mb-6">
        <div class="flex items-center gap-2 text-sm">
            <a href="{{ route('admin.monitoring.index') }}" class="text-gray-400 hover:text-blue-500">
    ...
    <div class="grid grid-cols-2 gap-6">
        ...
        {{-- Kiri: Foto Terbaru & Rincian --}}
        <div>
            {{-- Foto Terbaru --}}
            <div class="rounded-xl overflow-hidden bg-gray-100 mb-4 h-52 cursor-pointer"
                 onclick="openLightbox('{{ $project->progress->last()?->photos->first() ? asset('storage/'.$project->progress->last()->photos->first()->file_path) : '' }}')">
                @if($project->progress->last()?->photos->first())
                    ...
                    <div class="absolute inset-0 flex items-center justify-center opacity-0 hover:opacity-100 transition">
                        <span class="bg-black bg-opacity-40 text-white text-xs px-3 py-1 rounded-full">🔍 Perbesar</span>
                    </div>
                @else
        ...
        {{-- Kanan: Update Progres --}}
        <div>
            <div class="bg-gray-50 rounded-xl p-4 overflow-y-auto max-h-96">
                ...
                @forelse($project->progress as $progress)
                    <div class="mb-5 border-b border-gray-200 pb-4">
                        <p class="text-xs font-semibold text-gray-600 mb-1">
                            📅 {{ $progress->title }} (Progres {{ $progress->percentage }}%)
                        </p>
                        <p class="text-xs text-gray-500 leading-relaxed mb-2">{{ $progress->description }}</p>

                        {{-- Foto per progres --}}
                        @if($progress->photos->count())
