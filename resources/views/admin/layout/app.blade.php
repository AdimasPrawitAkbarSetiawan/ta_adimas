...
<head>
    ...
    <title>@yield('title', 'Admin') — SIMP-PRO</title>
...
<body>
    ...
    <div id="chat-popup" class="hidden">
        ...
        <div class="flex items-center justify-between px-4 py-3 bg-blue-600 text-white flex-shrink-0">
            <div class="flex items-center gap-2" id="chat-popup-header">
            ...
            <div class="flex items-center gap-2">
                ...
                <button onclick="closeChatPopup()"
                    class="text-white hover:text-blue-200 text-lg leading-none">✕</button>
            </div>
        </div>
        ...
        {{-- Area Pesan (tersembunyi dulu) --}}
        <div id="chat-messages-wrap" class="hidden flex-1 flex flex-col overflow-hidden">
    ...
    <div class="sidebar">
        ...
        <div>
            <div class="nav-item {{ request()->routeIs('admin.user.*') ? 'active' : '' }}"
                ...
                Kelola & Tambah User
                <svg class="ml-auto" style="width:11px;height:11px" fill="none" viewBox="0 0 24 24"
                    stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7" />
                </svg>
            </div>
            <div id="user-menu" class="nav-sub {{ request()->routeIs('admin.user.*') ? '' : 'hidden' }}">
                <a href="{{ route('admin.user.index') }}"
                    class="nav-item {{ request()->routeIs('admin.user.index') ? 'active' : '' }}">
                    Kelola User
                </a>
                <a href="{{ route('admin.user.create') }}"
                    class="nav-item {{ request()->routeIs('admin.user.create') ? 'active' : '' }}">
                    Tambah User
                </a>
            </div>
        </div>
            ...
            <div class="flex items-center gap-3">
                ...
                <div class="relative">
                    ...
                    <div id="notif-dropdown"
                        class="hidden absolute right-0 mt-2 w-80 bg-white rounded-2xl shadow-lg z-50 overflow-hidden">
                        <div class="flex items-center justify-between px-4 py-3 border-b border-gray-100">
                            <span class="text-sm font-semibold text-gray-700">Notifikasi</span>
                            <button onclick="markAllRead()" class="text-xs text-blue-600 hover:underline">Tandai semua
                                dibaca</button>
                        </div>
    ...
    <div class="px-4 py-3 border-b border-gray-50 ${n.is_read ? '' : 'bg-blue-50'} ${n.url ? 'cursor-pointer hover:bg-gray-100' : ''}"
         ${n.url ? `onclick="window.location.href='${n.url}'"` : ''}>
        <p class="text-xs font-semibold text-gray-700">${n.title}</p>
        ...
        ${n.url ? '<p class="text-xs text-blue-500 mt-1">Klik untuk lihat →</p>' : ''}
    </div>
    `).join('');
            });
        }
        ...
        function loadChatUsers() {
            fetch('/chat/users')
                ...
                <div onclick="openChatWith(${u.id}, '${u.name}', '${u.avatar_url}')"
                     class="flex items-center gap-3 px-4 py-3 hover:bg-gray-50 cursor-pointer border-b border-gray-50 transition">
                    <img src="${u.avatar_url || ''}"
                         class="w-8 h-8 rounded-full object-cover flex-shrink-0"
                         onerror="this.src='https://ui-avatars.com/api/?name=${encodeURIComponent(u.name)}&background=3b5bdb&color=fff&size=32'">
                    <div>
                        <p class="text-xs font-medium text-gray-700">${u.name}</p>
        ...
        function openChatWith(userId, userName, userAvatar) {
            ...
            document.getElementById('chat-messages-wrap').classList.remove('hidden');
            document.getElementById('chat-messages-wrap').style.display = 'flex';
            document.getElementById('chat-messages-wrap').style.flexDirection = 'column';
            document.getElementById('chat-messages-wrap').style.flex = '1';
            document.getElementById('chat-messages-wrap').style.overflow = 'hidden';
        ...
        function backToUserList() {
            ...
            document.getElementById('chat-messages-wrap').classList.add('hidden');
            loadChatUsers();
        }

        function loadChatMessages() {
            if (!activeChatUserId) return;
            fetch(`/chat/messages/${activeChatUserId}`)
                .then(r => r.json())
                .then(messages => {
                    ...
                    <div style="display:flex;justify-content:${isMine ? 'flex-end' : 'flex-start'}">
                        <div>
                            <div style="background:${isMine ? '#3b5bdb' : '#f3f4f6'};color:${isMine ? 'white' : '#374151'};padding:6px 10px;border-radius:12px;font-size:11px;max-width:200px;word-break:break-word;">
                                ${msg.message}
                            </div>
                            <p style="font-size:9px;color:#9ca3af;margin-top:2px;text-align:${isMine ? 'right' : 'left'}">${time}</p>
                        </div>
                    </div>`;
                    }).join('');
