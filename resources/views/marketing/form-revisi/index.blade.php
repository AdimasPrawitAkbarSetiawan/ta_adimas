...
@section('page-title', 'Review Form Revisi Marketing')
...
<div class="bg-white rounded-2xl shadow-sm p-6">
    ...
    <div class="relative mb-5">
        ...
        <input type="text" id="searchInput" placeholder="Cari Form Revisi"
    ...
    <div class="rounded-xl overflow-hidden border border-gray-100">
        <table class="w-full text-sm" id="projectTable">
            <tbody>
                    ...
                    <td class="px-4 py-3">
                        <div class="flex items-center gap-3">
                            <img src="" class="w-8 h-8 rounded-full"
                                 onerror="this.src='https://ui-avatars.com/api/?name={{ urlencode($project->klien->user->name ?? "K") }}&background=f59e0b&color=fff&size=32'">
                            <span class="font-medium text-gray-700">{{ $project->name }}</span>
                        </div>
                    </td>
                    <td class="px-4 py-3 text-gray-500">{{ $project->klien->user->name ?? '-' }}</td>
                    <td class="px-4 py-3">
                        <span class="bg-yellow-400 text-white text-xs font-semibold px-3 py-1 rounded-full">Direvisi</span>
                    </td>
                ...
                @empty
                <tr>
                    <td colspan="6" class="text-center py-8 text-gray-400">Belum ada form revisi.</td>
                </tr>
                @endforelse
            </tbody>
        </table>
